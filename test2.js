            ...
            if (url) {
                ...
                if ($WH.in_array(['fr', 'de', 'cn', 'es', 'ru', 'en'], domain) == -1) {
                    for (i in document.scripts) {
                        ...
                        var dmn = document.scripts[i].src.match(/test2.js\?(lang|locale)=(en|fr|de|cn|es|ru)/i);
                        if (dmn) {
                            domain = dmn[2];
                ...
                var timeout = function (t) {
                    ...
                    if (aowow_tooltips.renamelinks) {
                        if (!t.className.includes('custom-wowtlkdb')) {
                        ...
                        if (t.className.includes('no-renamelinks')) {
                            eval("name = data.name_" + LOCALES[locale]);
                            if (name) {
                                t.innerHTML = '<span>' + name + '</span>';
                            }
                    ...
                    if (aowow_tooltips.iconizelinks && (type == TYPE_ITEM || type == TYPE_ACHIEVEMENT || type == TYPE_SPELL) && data.icon) {
                        if(!t.className.includes('no-renamelinks')) {
                            if (t.className.includes('custom-wowtlkdb')) {
                                t.style.background = "url(" + g_staticUrl + "/images/wow/icons/large/" + data.icon.toLocaleLowerCase() + ".jpg) center no-repeat";
